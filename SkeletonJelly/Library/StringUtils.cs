...
public static class StringUtils {
	...
	*/
	...
	public static string Format(this float f, int dec) {
		...
		float fr = f - ir;
		string s = "";
		if (dec > 0 && ir == 0 && fr < Math.Pow(10, -dec)) {
			s = "0.";
			if (f < 0) { s = "-" + s; }
		...
		if (dec > 0 && fr > 0) {
			s = "" + fr;
			if (s.Length <= dec + 1) { s = s.Substring(1); } else { s = s.Substring(1, dec + 1); }
		}
		return (neg ? "-" : "") + ir + s;
	}
	...
	public static string ShortString(this float f, int places = 0) {
		...
		return (neg ? "-" : "") + Math.Round(val * 1000f) / 1000f + notationValue;
	}
	...
	public static string TimeFormat(this float f, int places) {
		float hr = (float)Math.Floor((f / 3600.0f));
		float min = (float)Math.Floor((f / 60.0f) % 60.0f);
